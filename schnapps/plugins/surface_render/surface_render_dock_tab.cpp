	...
	if (!updating_ui_)
	{
		...
		if (view && map)
		{
			...
			if (p.use_transparency_)
			{
				...
				{
					if (b)
						plugin_surface_render_transp::add_tr_flat(plugin_->plug_transp_,view,map,p.get_transp_flat_param());
					else
						plugin_surface_render_transp::remove_tr_flat(plugin_->plug_transp_,view,map,p.get_transp_flat_param());
				}
				if (p.face_style_== MapParameters::PHONG)
				{
					if (b)
						plugin_surface_render_transp::add_tr_phong(plugin_->plug_transp_,view,map,p.get_transp_phong_param());
					else
						plugin_surface_render_transp::remove_tr_phong(plugin_->plug_transp_,view,map,p.get_transp_phong_param());
				}
			}
#endif
			view->update();
